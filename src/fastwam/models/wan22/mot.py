...
class MoT(nn.Module):
    ...
    def forward(
        ...
        """Run the shared multi-expert self-attention over all experts.

        Args:
            detach_video_for_action: legacy 2-expert shortcut used by
                :class:`FastWAM`. Only fires when ``len(expert_order) == 2``
                with order ``['video', 'action']``.
            detach_kv_experts: optional set of expert names whose **K/V**
                should be detached *when other experts attend to them*.
                The expert's own Q attending to its own K/V is **not**
                detached, so its own loss-driven gradient still updates its
                own QKV weights. This is the knowledge-insulation knob used
                by H-FastWAM to prevent the video / action losses from
                flowing into the language expert. When set, attention is
                computed per-Q-expert so each Q-expert sees the correct
                (selectively detached) K/V stack.
        """
        ...
        if detach_kv_experts is not None:
            unknown = set(detach_kv_experts) - set(self.expert_order)
            if unknown:
                raise ValueError(
        ...
        for layer_idx in range(self.num_layers):
            ...
            if detach_video_for_action and len(self.expert_order) == 2 and self.expert_order[0] == "video" and self.expert_order[1] == "action":
                # Split attention: video queries get full gradients, action queries get detached video K/V
                video_seq_len = seq_lens[0]

                # Video queries attend to everything (full gradient for video loss)
                ...
                # Per-Q-expert attention with selective K/V detach.
                # For Q-expert `name_q`, K/V of every other expert `name_kv`
                # in `detach_kv_experts` is detached; `name_q`'s own K/V is
                # never detached (so its own self-attention still trains).
                detach_set = set(detach_kv_experts)
                per_expert_mixed = []
                q_start = 0
                for q_idx, name_q in enumerate(self.expert_order):
                    ...
                    k_pieces = []
                    v_pieces = []
                    for kv_idx, name_kv in enumerate(self.expert_order):
