...
class WanVideoDiT(torch.nn.Module):
    ...
    def build_video_to_video_mask(
        ...
        if self.video_attention_mask_mode == "first_frame_causal":
            # Condition frames (latent frames 0 to n_cond-1) cannot attend to future generated frames.
            # Generated frames can attend to everything (condition + other generated frames).
            # With overhead_conditioning, n_cond_latent_frames=3 by default.
            n_cond = getattr(self, '_n_cond_latent_frames_for_mask', 1)
            cond_tokens = min(n_cond * video_tokens_per_frame, video_seq_len)
            video_mask = torch.ones((video_seq_len, video_seq_len), dtype=torch.bool, device=device)
            # Condition tokens cannot see generated tokens
            video_mask[:cond_tokens, cond_tokens:] = False
            return video_mask
    ...
    def pre_dit(
        ...
        batch_size = x.shape[0]
        # Store n_cond_latent_frames for mask building
        self._n_cond_latent_frames_for_mask = n_cond_latent_frames
        ...
        if self.seperated_timestep and fuse_vae_embedding_in_latents:
            ...
            # All condition latent frames get timestep=0 (clean/frozen)
            token_timesteps[:, :n_cond_latent_frames, :] = 0
