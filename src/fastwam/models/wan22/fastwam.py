...
class FastWAM(torch.nn.Module):
    ...
    def build_inputs(self, sample, tiled: bool = False):
        ...
        # Determine number of condition latent frames
        n_cond_frames = sample.get("n_cond_frames", 9)
        if isinstance(n_cond_frames, torch.Tensor):
            n_cond_frames = n_cond_frames[0].item()
        n_cond_frames = int(n_cond_frames)
        # Convert pixel condition frames to latent condition frames
        # VAE temporal compression: T_lat = (T + 3) // 4
        # For 9 condition frames: (9+3)//4 = 3 latent frames
        n_cond_latent_frames = (n_cond_frames + 3) // 4
        ...
        return {
            ...
            "overhead_latent": overhead_latent,
    ...
    def _build_mot_attention_mask(
        ...
        # action -> ALL condition frame video tokens (not just first frame)
        cond_tokens = min(n_cond_latent_frames * video_tokens_per_frame, video_seq_len)
    ...
    def training_loss(self, sample, tiled: bool = False):
        ...
        n_cond_latent_frames = inputs["n_cond_latent_frames"]
        overhead_latent = inputs["overhead_latent"]
        ...
        pred_video = pred_video[:, :, n_cond_latent_frames:]
        target_video = target_video[:, :, n_cond_latent_frames:]

        # For the new multi-frame architecture, we don't use image_is_pad for video loss
        # because future frames are clamped to episode end (not zero-padded).
        # All generated latent frames are valid targets.
        ...
        # Optional stop prediction loss (uses action_is_pad as supervision)
        if "stop" in pred_action_dict and action_is_pad is not None:
            stop_logits = pred_action_dict["stop"].squeeze(-1)  # [B, T]
            stop_labels = action_is_pad.float()  # [B, T] — padded steps = reached goal = stop=1
