...
class Wan22Trainer:
    def __init__(self, model, train_dataset, val_dataset=None, *, cfg: DictConfig):
        ...
        # Build parameter groups — projection gets a separate (higher) learning rate
        ...
        if getattr(self.model, "use_visual_encoder", False) and not self.freeze_visual_encoder:
            proj_params = list(self.model.visual_encoder.projection.parameters())
            if proj_params:  # skip_projection=True → nn.Identity() → no params
                ...
                logger.info(
                    "Projection LR: %.2e (%.1fx base LR %.2e)",
                    projection_lr, self.projection_lr_multiplier, self.learning_rate,
                )
            else:
                logger.info(
